...
class Image_reader():
            ...
            while (line):
                ...
                with open(os.path.join(self.root_dir,line,'groundtruth.txt')) as f2:
                    ...
                    while (line2):
                        box=line2.split(',')
                        if len(box) > 4:
                            minx = min(float(box[0]), float(box[2]), float(box[4]), float(box[6]))
                            maxx = max(float(box[0]), float(box[2]), float(box[4]), float(box[6]))
                            miny = min(float(box[1]), float(box[3]), float(box[5]), float(box[7]))
                            maxy = max(float(box[1]), float(box[3]), float(box[5]), float(box[7]))

                            boxes.append([int(minx), int(miny), int(maxx-minx), int(maxy-miny)])
                        else:
    ...
    def get_data(self,batch_size=1):
        ...
        if index_d < 0 or index_d >= len(img_list):
            index_d = np.random.choice(range(len(img_list)//2))
